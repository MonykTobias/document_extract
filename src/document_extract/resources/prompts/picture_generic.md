You are transcribing ONE image cropped from a report page.
...
- table: transcribe ALL visible text and numbers as one or more markdown pipe tables that mirror the visible rows, columns and groups. Repeat a group label on every row it spans. Include any visible title line and total/summary lines as regular rows.
- kpi: output one line per KPI as `- LABEL: value`, pairing each display figure with the caption printed next to it. Keep visible group headings.
